#!/usr/bin/env python3
"""
Добавляет в cleaned_items.json базовые предметы из update_item_stats.py,
которых там ещё нет.
"""
...
# Читаем stats_map из update_item_stats.py
def extract_stats_map():
    with open(STATS_PATH, "r", encoding="utf-8") as f:
        content = f.read()
    # Находим stats_map = { ... }
    # Простой подход: выполнить код в ограниченной среде
    namespace = {}
    exec(content, namespace)
    return namespace.get("stats_map", {})

def convert_price(price_gold_float):
    """Конвертирует float цену в (gold, silver)"""
...
def main():
    ...
    # Загружаем существующий cleaned_items.json
