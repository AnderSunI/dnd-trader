#!/usr/bin/env python3
"""
Финальный парсер BG3 wiki v2.
- Вытаскивает damage (одноручный и двуручный)
- Вытаскивает AC
- Чистит rarity
- Определяет category
"""
...
def extract_damage(text):
    """Ищет строки урона и возвращает словарь"""
    damage = {}
    # Одноручный урон
    one = re.search(r'One-handed damage\s*([\dd\+\-\s]+)\s*(\w+)?', text, re.IGNORECASE)
    if one:
        dice = one.group(1).strip()
        dmg_type = one.group(2) if one.group(2) else ""
        damage["one_handed"] = {"dice": dice, "type": dmg_type}
    # Двуручный урон
    two = re.search(r'Two-handed damage\s*([\dd\+\-\s]+)\s*(\w+)?', text, re.IGNORECASE)
    if two:
        dice = two.group(1).strip()
        dmg_type = two.group(2) if two.group(2) else ""
        damage["two_handed"] = {"dice": dice, "type": dmg_type}
    return damage
...
def parse_item_page(url: str, session: requests.Session) -> Optional[Dict]:
    ...
    armor_type = None
    if "Heavy Armour" in full_text:
        armor_type = "heavy"
    elif "Medium Armour" in full_text:
        armor_type = "medium"
    elif "Light Armour" in full_text:
        armor_type = "light"
    ...
    for element in main_content.find_all(["p", "ul", "div", "li"]):
        ...
        # Простые свойства (Light, Finesse, Thrown, Two-Handed, Versatile)
        props_found = []
        ...
        # Описание – первый длинный параграф, не начинающийся с ключевых слов
        if not item["description"] and text and not re.match(r'^(Rarity|Weight|Price|Damage|Armour Class|Enchantment|One-handed|Two-handed):', text, re.IGNORECASE):
            if len(text) > 30:
...
def main():
    ...
    with open("bg3_final.json", "w", encoding="utf-8") as f:
