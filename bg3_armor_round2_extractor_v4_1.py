...
def strip_html_tags_fast(fragment: str) -> str:
    if not fragment:
        return ""
...
def extract_flavor_from_html(html_text: str) -> str:
    ...
    fast_patterns = [
        re.compile(
            r'<div[^>]*style="[^"]*font-family\s*:\s*Kurale[^"]*"[^>]*>(.*?)</div>',
            flags=re.I | re.S,
        ),
        re.compile(
            r'<div[^>]*style="[^"]*border-left[^"]*"[^>]*>(.*?)</div>',
            flags=re.I | re.S,
        ),
    ]
    for pattern in fast_patterns:
        ...
        if match:
            text = strip_html_tags_fast(match.group(1))
            if len(text) >= 20:
                return text

    marker = re.search(r'Описание в игре', html_text, flags=re.I)
    if marker:
        window_start = max(0, marker.start() - 4000)
        fragment = html_text[window_start:marker.start()]
        div_matches = list(re.finditer(r'<div[^>]*>(.*?)</div>', fragment, flags=re.I | re.S))
        for match in reversed(div_matches[-6:]):
            text = strip_html_tags_fast(match.group(1))
            if len(text) >= 20:
                return text
...
def extract_section_lines_from_html(html_text: str) -> List[str]:
    ...
    for section_name in SECTION_NAMES:
        pattern = re.compile(
            rf'<h([1-6])[^>]*>.*?{re.escape(section_name)}.*?</h\1>(.*?)(?=<h[1-6][^>]*>|\Z)',
            flags=re.I | re.S,
        )
    ...
    for fragment in section_fragments:
        ...
        for node in soup.find_all(["ul", "ol", "p"], recursive=True):
