...
def main():
    # Пробуем найти BG3 JSON в разных местах, приоритет bg3_items_clean.json
    candidates = [
        Path('bg3_items_clean.json'),
        Path('data/bg3_items_clean.json'),
        Path('bg3_clean.json'),
        Path('data/bg3_clean.json'),
        Path('bg3_final_with_stats.json'),
        Path('data/bg3_final_with_stats.json'),
    ]
    input_file = None
    for candidate in candidates:
        if candidate.exists():
            ...
            break
    if not input_file:
        print("Файл BG3 не найден. Проверьте, есть ли в корне или data/ файл bg3_items_clean.json или аналогичный.")
        return

    data = load_json(input_file)
    print(f"Загружено {len(data)} предметов из {input_file}")
    ...
    output_file = Path('bg3_items_normalized.json')
