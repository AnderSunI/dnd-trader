...
"""
BG3 API Weapons Round1 Driver v3
...
Запуск:
  python3 bg3_api_weapons_round1_driver_v3.py
"""
...
TIMEOUT = 60
USER_AGENT = "DNDTraderBG3WeaponsDriver/0.3 (+local testing; respectful requests)"
...
WEAPONISH_HINTS = [
    "меч", "топор", "булава", "цеп", "молот", "рапира", "серп", "скимитар",
    "трезубец", "пика", "копье", "копьё", "глефа", "дубинка", "дубина",
    "кинжал", "лук", "арбалет", "пилум", "посох", "клинок", "секач", "кирка",
    "фонарь", "факел", "салями", "ветвь", "шприц", "скальпель", "катана",
]
...
def collect_items_from_table(table: Tag, base_url: str, subcategory: str, page_label: str) -> List[Dict[str, str]]:
    ...
    for row in rows:
        # Prefer first meaningful link in row that looks like item page.
        found = False
        for a in row.find_all("a", href=True):
            ...
            if not is_weaponish_title(title):
                # For safety, row-first parser only accepts weapon-ish titles.
                continue
        ...
        # Fallback for weird rows: any link with bold-ish title cells.
...
def build_report(payload: Dict[str, object]) -> str:
    lines: List[str] = []
    lines.append("BG3 API Weapons Round1 Driver v3")
    lines.append("================================")
